...
#####################Manipuation on single path#######################
def setPathMaterial(pathEl,id):
    if (len(pathEl.findall("materialid"))):
       pathEl.findall("materialid")[0].text = "%f"%id

def getMaterial(fabTree):
    #root = fabTree.getroot()
    palette = fabTree.findall("palette")[0]
    matEl = palette.find("material")
    return matEl
